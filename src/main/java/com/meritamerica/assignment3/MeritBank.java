...
public class MeritBank {
		...
		try (BufferedWriter nextLine = new BufferedWriter(new FileWriter(fileName))) {
			...
			for (int a = 0; a < accountHolderArray.length; a++) {
				...
				for (int c = 0; c < accountHolderArray[a].getCheckingAccounts().length; c++) {
					...
					nextLine.newLine();
					nextLine.write(
							String.valueOf(accountHolderArray[a].getCheckingAccounts()[c].getTransactions().size()));
					nextLine.newLine();
					int ctl = accountHolderArray[a].getCheckingAccounts()[c].getTransactions().size();
					for (int ct = 0; ct < ctl; ct++) {
						nextLine.write(accountHolderArray[a].getCheckingAccounts()[c].getTransactions().get(ct)
								.writeToString());
						nextLine.newLine();
					}
				}
				...
				for (int s = 0; s < accountHolderArray[a].getSavingsAccounts().length; s++) {
					...
					nextLine.newLine();
					nextLine.write(
							String.valueOf(accountHolderArray[a].getSavingsAccounts()[s].getTransactions().size()));
					nextLine.newLine();
					int stl = accountHolderArray[a].getSavingsAccounts()[s].getTransactions().size();
					for (int st = 0; st < stl; st++) {
						nextLine.write(accountHolderArray[a].getSavingsAccounts()[s].getTransactions().get(st)
								.writeToString());
						nextLine.newLine();
					}
				}
				...
				for (int cd = 0; cd < accountHolderArray[a].getCDAccounts().length; cd++) {
					...
					for (int cdt = 0; cdt < cdtl; cdt++) {
						nextLine.write(
								accountHolderArray[a].getCDAccounts()[cd].getTransactions().get(cdt).writeToString());
						nextLine.newLine();
					}
				}
			}
	...
	public static boolean processTransaction(Transaction transaction)
			throws NegativeAmountException, ExceedsAvailableBalanceException, ExceedsFraudSusLimitException {
		...
		if (source == null) {
			if (transaction instanceof WithdrawTransaction) {
				...
				if (transaction.getAmount() < -1000) {
					...
					throw new ExceedsFraudSusLimitException(
